wait(5)
Browser("Advantage Shopping").Page("Advantage Shopping").Link("UserMenu").Click @@ script infofile_;_ZIP::ssf37.xml_;_
wait(2)
Browser("Advantage Shopping").Page("Advantage Shopping").Link("CREATE NEW ACCOUNT").Click @@ script infofile_;_ZIP::ssf38.xml_;_
Browser("Advantage Shopping").Page("Advantage Shopping").WebEdit("usernameRegisterPage").Set "aakash" @@ script infofile_;_ZIP::ssf39.xml_;_
Browser("Advantage Shopping").Page("Advantage Shopping").WebElement("*EmailEmail").Click @@ script infofile_;_ZIP::ssf40.xml_;_
Browser("Advantage Shopping").Page("Advantage Shopping").WebElement("Email").Click @@ script infofile_;_ZIP::ssf41.xml_;_
Browser("Advantage Shopping").Page("Advantage Shopping").WebEdit("emailRegisterPage").Set "aakash@gmail.com" @@ script infofile_;_ZIP::ssf42.xml_;_
Browser("Advantage Shopping").Page("Advantage Shopping").WebEdit("passwordRegisterPage").SetSecure "686ce563cf3ee03b5da2c79b9bf3c85a74a1c14fc56d2e79" @@ script infofile_;_ZIP::ssf43.xml_;_
Browser("Advantage Shopping").Page("Advantage Shopping").WebElement("Confirm password").Click @@ script infofile_;_ZIP::ssf44.xml_;_
Browser("Advantage Shopping").Page("Advantage Shopping").WebEdit("confirm_passwordRegisterPage").SetSecure "686ce56849b1532cf39caeb74aa8777a9d7bf599bcc3c0e6" @@ script infofile_;_ZIP::ssf45.xml_;_
Browser("Advantage Shopping").Page("Advantage Shopping").WebCheckBox("i_agree").Set "ON" @@ script infofile_;_ZIP::ssf46.xml_;_
Browser("Advantage Shopping").Page("Advantage Shopping").WebButton("register_btn").Click @@ script infofile_;_ZIP::ssf47.xml_;_
Browser("Advantage Shopping").Page("Advantage Shopping").Sync
Browser("Advantage Shopping").Back @@ hightlight id_;_198482_;_script infofile_;_ZIP::ssf48.xml_;_
Reporter.ReportEvent micFail,"Register","User not created"

 @@ script infofile_;_ZIP::ssf32.xml_;_
 @@ script infofile_;_ZIP::ssf36.xml_;_
